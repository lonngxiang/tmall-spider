...
class TmallSpider(scrapy.Spider):
    ...
    def start_requests(self):
        word_lists = ["资生堂","迪奥","兰蔻","雅诗兰黛","SKII","香奈儿","CPB","IPSA","NARS","怡丽丝尔","安热沙"]
    ...
    def parse1(self, response):
        ...
        print(len(html1.xpath('//div[@class="product  "]')))
        print("++++++++++++++++++")
        # for i in range(1):
        for i in range(len(html1.xpath('//div[@class="product  "]'))):
            product_url = html1.xpath(
                '//div[@class="product  "][{}]//*[@class="productTitle" or contains(@class,"productTitle")]/a[1]/@href'.format(
                    i + 1))[0]
            print(product_url)
            ...
            product_title1 = html1.xpath(
                '//div[@class="product  "][{}]//*[@class="productTitle" or contains(@class,"productTitle")]/a[1]//text()'.format(
                    i + 1))
            product_title = "".join(product_title1).strip()
            print(product_title)
    ...
    def parse(self, response):
        ...
        for w in range(len(new_html2['rateDetail']['rateList'])):
            ...
            try:
                ...
                # response=requests.post(url="http://servic.com/api/v1/es/bulk",headers=headers2,json=json.dumps(lists_all,ensure_ascii=False))
                response = requests.post(url="https://servicom/api/v1/es/bulk/tmall", headers=headers2,
                                         json=cotents_all)
